...
export default function CustomerPage() {
  ...
  const fetchCart = async () => {
    ...
    try {
      const response = await axios.get(
        `http://72.60.202.205:5173/api/customer/cart?customerId=${customerId}`,
  ...
  const fetchWishlist = async () => {
    ...
    try {
      const response = await axios.get(
        `http://72.60.202.205:5173/api/customer/wishlist?customerId=${customerId}`,
        { headers: { Origin: "http://localhost:5173" } }
      );
      setWishlist(
  ...
  const handleToggleWishlist = async (productId) => {
    ...
    try {
      const response = await axios.post(
        "http://72.60.202.205:5173/api/customer/wishlist",
  ...
  const updateQuantity = async (variantId, change) => {
    ...
    try {
      await axios.put(
        "http://72.60.202.205:5173/api/customer/cart",
