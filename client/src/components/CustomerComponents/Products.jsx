...
function ProductCard({
  ...
  return (
    ...
    >
     ...
     {/* Updated Price Display with Actual Price + Strikeout */}
<div className="flex items-start justify-between pt-1 flex-wrap gap-4">
  <div className="flex flex-col items-start flex-shrink-0">
    ...
    {/* Actual Price with Strikeout + Discount % (if applicable) */}
    {effectiveVariant?.actual_price && 
     Number(effectiveVariant.actual_price) > Number(effectiveVariant.price) && (
      <div className="flex items-center gap-2 text-xs mt-0.5">
        <span className="line-through text-gray-400">
          ₹{Number(effectiveVariant.actual_price).toFixed(2)}
        </span>
        {effectiveVariant.discount_percentage && Number(effectiveVariant.discount_percentage) > 0 && (
          <span className="text-green-600 font-medium">
            ({Number(effectiveVariant.discount_percentage).toFixed(0)}% off)
          </span>
        )}
      </div>
    )}

    {/* Total Price when item is in cart */}
    {quantity > 0 && (
  ...
  {/* Cart Quantity Controls - Unchanged */}
