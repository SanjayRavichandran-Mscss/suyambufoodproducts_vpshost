...
//       try {
//         const response = await fetch('https://suyambufoods.com/api/customer/wishlist?customerId=1');
//         if (!response.ok) {
//           throw new Error('Failed to fetch wishlist');
//         }
//         const data = await response.json();
//         setWishlist(data.wishlist || []);
//         setTotalWished(data.totalWished || 0);
//       } catch (err) {
...
//                     style={{ fontSize: '1.5em', color: item.is_liked ? '#e74c3c' : '#bdc3c7' }}
//                   >
//                     ❤️
//                   </div>
//                 </div>
  ...
  useEffect(() => {
    const fetchWishlist = async () => {
      ...
      try {
        ...
        const data = await response.json();
        setWishlist(data.wishlist || []);
        const count = data.totalWished || 0;
