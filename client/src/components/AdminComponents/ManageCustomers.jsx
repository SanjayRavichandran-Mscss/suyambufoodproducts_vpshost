...
//       try {
//         const response = await fetch("https://suyambuoils.com/api/admin/customers");
...
//       result = result.filter(customer => {
//         return customer.created_at && new Date(customer.created_at) >= oneWeekAgo;
//       });
...
//     // Safe sorting by username (null/undefined → treated as empty string)
//     result.sort((a, b) => {
//       const nameA = String(a?.username ?? "").trim().toLowerCase();
//       const nameB = String(b?.username ?? "").trim().toLowerCase();
...
//     setErrorOrders(null);
//     try {
//       const response = await fetch(`https://suyambuoils.com/api/customer/orders?customerId=${customerId}`);
...
//     const statusText = (status || "unknown").replace(/_/g, " ").toUpperCase();
//     const colorMap = {
...
//               <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-green-800 truncate">
...
//             {loadingOrders ? (
//               <div className="text-center text-green-700 text-base sm:text-lg lg:text-xl font-semibold">
//                 Loading orders...
//               </div>
//             ) : errorOrders ? (
//               <div className="text-center text-red-600 text-base sm:text-lg lg:text-xl font-semibold">
//                 Error: {errorOrders}
//               </div>
//             ) : orders.length === 0 ? (
...
//                             <span>Total: ₹{parseFloat(order.total_amount || 0).toFixed(2)}</span>
//                           </p>
//                           <p className="flex items-center text-gray-700">
...
//                                   Price: ₹{parseFloat(item.price_at_purchase || 0).toFixed(2)}
//                                 </p>
//                                 <p className="text-xs text-gray-600">
...
import React, { useState, useEffect } from "react";
import { Filter, Search, ArrowDownUp, Clock, MapPin, Package, DollarSign, X, User, Mail, Phone, Calendar, FileText } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
...
const ManageCustomers = () => {
  ...
  const [filteredCustomers, setFilteredCustomers] = useState([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [sortOrder, setSortOrder] = useState("newest"); // changed default to newest
  const [filterType, setFilterType] = useState("all");
  const [customMonth, setCustomMonth] = useState("");
  const [showFilterOptions, setShowFilterOptions] = useState(false);
  const [selectedCustomerId, setSelectedCustomerId] = useState(null);
  ...
  useEffect(() => {
    ...
    // Apply date filter (registration date)
    const now = new Date();
    if (filterType === "week") {
      const oneWeekAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
      result = result.filter(customer => {
    ...
    // Sort by registration date (created_at)
    result.sort((a, b) => {
          ...
          <AnimatePresence>
            {showFilterOptions && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                className="absolute z-10 mt-1 sm:mt-2 w-full lg:w-56 bg-white border border-green-200 rounded-md sm:rounded-lg shadow-lg p-2"
              >
                <button
                  onClick={() => handleSortChange("newest")}
                  className="flex items-center gap-2 w-full px-3 py-2 hover:bg-green-50 text-green-700 text-sm"
                >
                  <ArrowDownUp size={16} />
                ...
                <button
                  onClick={() => handleFilterTypeChange("all")}
                  className="w-full text-left px-3 py-2 hover:bg-green-50 text-green-700 text-sm"
                >
                  All customers
                </button>
                <button
          ...
          >
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <User size={20} className="text-green-600" />
                  <span className="font-semibold text-lg text-green-800">S.No {index + 1}</span>
                </div>
                <div className="flex items-center gap-1 text-xs text-gray-500">
                  <Calendar size={14} />
                  <span>
      ...
      <div className="hidden lg:block">
        <div className="overflow-x-auto rounded-lg shadow-lg">
          <table className="min-w-full border-collapse border border-green-200">
            <thead>
              <tr className="bg-green-50">
                <th className="border border-green-200 px-4 py-3 text-left text-green-800 text-sm font-semibold">S.No</th>
                <th className="border border-green-200 px-4 py-3 text-left text-green-800 text-sm font-semibold">Username</th>
                <th className="border border-green-200 px-4 py-3 text-left text-green-800 text-sm font-semibold">Email</th>
                <th className="border border-green-200 px-4 py-3 text-left text-green-800 text-sm font-semibold">Full Name</th>
                <th className="border border-green-200 px-4 py-3 text-left text-green-800 text-sm font-semibold">Phone</th>
            ...
            <tbody>
              {filteredCustomers.map((customer, index) => (
                <tr key={customer.id} className="hover:bg-green-50 transition-colors">
                  <td className="border border-green-200 px-4 py-3 text-sm font-medium">{index + 1}</td>
                  <td className="border border-green-200 px-4 py-3 text-sm font-medium">{customer.username || "—"}</td>
                  <td className="border border-green-200 px-4 py-3 text-sm break-all max-w-xs">{customer.email || "—"}</td>
                  <td className="border border-green-200 px-4 py-3 text-sm break-words">{customer.full_name || "—"}</td>
                  <td className="border border-green-200 px-4 py-3 text-sm">{customer.phone || "—"}</td>
