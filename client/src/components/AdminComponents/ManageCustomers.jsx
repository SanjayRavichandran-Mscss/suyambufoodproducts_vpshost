...
const ManageCustomers = () => {
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        const response = await fetch("https://suyambufoods.com/api/api/admin/customers");
  ...
  const fetchOrders = async (customerId) => {
    ...
    setErrorOrders(null);
    try {
      const response = await fetch(`https://suyambufoods.com/api/api/customer/orders?customerId=${customerId}`);
                        ...
                        <div className="space-y-3 sm:space-y-4">
                          {order.items.map(item => (
                            <div key={item.product_id} className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-3 lg:space-x-4">
                              <img
                                src={`https://suyambufoods.com/api/api${item.thumbnail_url}`}
                                alt={item.name}
