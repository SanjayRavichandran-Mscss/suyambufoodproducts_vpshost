...
//       try {
//         const response = await fetch(`https://suyambufoods.com/api/api/admin/profile/${adminId}`);
...
export default function AdminMenus({ adminId }) {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        const response = await fetch(`https://suyambufoods.com/api/api/admin/profile/${plainId}`);
        const data = await response.json();
        if (response.ok) {
          setProfile(data);
        } else {
