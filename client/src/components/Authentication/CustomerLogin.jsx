...
export default function CustomerLogin({ onClose }) {
  const [showLogin, setShowLogin] = useState(false);     // ← Default: false → shows Register
  ...
  const handleSubmit = async e => {
    ...
    try {
      const res = await axios.post("https://suyambuoils.com/api/customer/login", form);
  ...
  const handleForgotClick = () => setShowForgot(true);

  if (showForgot) {
    return <CustomerPasswordReset onLoginClick={handleShowLogin} onClose={onClose} />;
  }

  // Default view = Register (Create Account)
  if (!showLogin) {
    return (
      <CustomerRegister
        onLoginClick={handleShowLogin}   // "Already have an account? Sign in"
        onClose={onClose}
      />
    );
  }

  // Login view (shown only after clicking "Sign in")
  return (
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {/* Link to switch back to Register */}
