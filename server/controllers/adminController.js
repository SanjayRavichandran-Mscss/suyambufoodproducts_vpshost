...
const IMAGE_BASE = "";  // Use relative paths (/productImages/...) – frontend/Nginx handles resolution
...
exports.updateProduct = [
  ...
  async (req, res) => {
    try {
      ...
      // Handle existing_additional_images if provided (JSON string)
      let parsedExisting = parseAdditionalImages(existing_additional_images ? JSON.parse(existing_additional_images) : product.additional_images);
      let newAdditional = req.files?.additional_images?.map(f => `/productImages/${f.filename}`) || [];
      let additional_images = [...parsedExisting, ...newAdditional];
...
exports.viewProducts = async (req, res) => {
  ...
  try {
    ...
    rows.forEach((r) => {
      if (!productsMap[r.id]) {
        let additionalImages = parseAdditionalImages(r.additional_images);
        // Use relative paths only (no prepending)
        const thumbnail_url = r.thumbnail_url || null;
        const additional_images = additionalImages.filter(Boolean);  // Filter nulls
        productsMap[r.id] = {
          id: r.id,
          name: r.name,
          description: r.description,
          ...
          stock_status: r.stock_status,
          variants: [],
          isBanner: r.isBanner || 0,  // Add if missing
          bannerimg: r.bannerimg || null,  // Add banner
        };
      }
      if (r.variant_id) {
        productsMap[r.id].variants.push({
  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT 
        ...
        s.status AS stock_status, s.id AS stock_status_id,
        pv.id AS variant_id, pv.variant_quantity, pv.price AS variant_price, 
        u.id AS variant_uom_id, u.uom_name, p.isBanner, p.bannerimg
      FROM products p
      LEFT JOIN categories c ON p.category_id = c.id
      LEFT JOIN stock_statuses s ON p.stock_status_id = s.id
      LEFT JOIN product_variants pv ON pv.product_id = p.id
    ...
    const product = {
      ...
      stock_status_id: rows[0].stock_status_id,
      stock_status: rows[0].stock_status,
      isBanner: rows[0].isBanner || 0,
      banner_url: rows[0].bannerimg || null,
      variants: [],
    };
...
exports.getBannerProducts = async (req, res) => {
  ...
  try {
    ...
    const bannerProducts = rows.map((product) => ({
      ...
      name: product.name,
      description: product.description,
      banner_url: product.bannerimg || null,  // Relative path only
    }));
