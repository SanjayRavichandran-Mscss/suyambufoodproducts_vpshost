...
exports.addProduct = [
  ...
  async (req, res) => {
    try {
      ...
      console.log(`🟢 Added product ${productId} with ${validVariants} variants`);
...
exports.updateProduct = [
  ...
  async (req, res) => {
    try {
      ...
      // Handle additional images with existing
      let existingAdditional = parseAdditionalImages(product.additional_images);
      ...
      const newAdditional = req.files?.additional_images?.map(f => `/productImages/${f.filename}`) || [];
      ...
      console.log(`🟢 Updated product ${id} with ${validVariants} variants`);
