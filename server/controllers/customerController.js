...
//     const templateData = {
//       baseUrl: process.env.BASE_URL || 'http://72.60.202.205:5173',
...
exports.placeOrder = async (req, res) => {
    ...
    try {
        ...
        let orderItemsValues = [];
        const variantIds = items.map(item => item.variantId);
        const [variantRows] = await db.query(`SELECT id, price FROM product_variants WHERE id IN (?)`, [variantIds]);
        const variantMap = variantRows.reduce((map, row) => ({ ...map, [row.id]: parseFloat(row.price) }), {});
        for (const item of items) {
            if (!item.variantId || !item.quantity || isNaN(item.quantity)) return res.status(400).json({ message: 'Invalid item data' });
            const qty = parseInt(item.quantity, 10);
            const price = variantMap[item.variantId];
            if (!price || qty <= 0) return res.status(400).json({ message: `Invalid variantId or quantity` });
            const subtotal = price * qty;
            calculatedTotal += subtotal;
            orderItemsValues.push([null, item.variantId, qty, price, subtotal]);
        }
        ...
        try {
            ...
            orderItemsValues = orderItemsValues.map(row => [orderId, ...row.slice(1)]);
            if (orderItemsValues.length > 0) await connection.query(`INSERT INTO order_items (order_id, product_variant_id, quantity, price_at_purchase, subtotal) VALUES ?`, [orderItemsValues]);
...
exports.getOrders = async (req, res) => {
    ...
    try {
        ...
        const [orders] = await db.query(ordersQuery, [parsedCustomerId]);
        const itemsQuery = 'SELECT oi.order_id, oi.product_variant_id, oi.quantity, oi.price_at_purchase, oi.subtotal, p.name, p.description, p.thumbnail_url, pv.variant_quantity, um.uom_name FROM order_items oi JOIN product_variants pv ON oi.product_variant_id = pv.id JOIN products p ON pv.product_id = p.id LEFT JOIN uom_master um ON pv.uom_id = um.id JOIN orders o ON oi.order_id = o.id WHERE o.customer_id = ?';
        const [itemsRows] = await db.query(itemsQuery, [parsedCustomerId]);
        const itemsMap = {};
        for (const row of itemsRows) {
            const orderId = row.order_id;
